...
4. **蓝图即可证伪对象**：每张图自带记分卡（样本内/验证窗/保真度衰减/成本证书/反事实稳健性），gallery 按证据排序而非感觉。
...
| 差异化创新 | **"诚实编译器"四件套全部采纳**（用户 2026-07-05 确认）：①因果类型系统 ②静态成本证书 ③反事实分叉 ④蓝图记分卡 |
...
- **反事实分叉（创新③）**：时间旅行视图中分叉任意历史 bar：改写指定节点输出（如"风控官没拦""LLM 说做空"）→ 平行世界确定性重放 → PnL 差值归因报告。Reflector 的过程/结局打分以此为证据源。降级保险丝：先做运行时分叉+CLI 报告，UI 右键交互后补。
...
- **蓝图记分卡（创新④）**：每张保存的蓝图自动附记分卡——样本内/验证窗表现、保真度阶梯衰减、成本证书、反事实稳健性摘要；gallery 按证据排序。
...
- **D4**：进化实验室+谱系树 + **反事实分叉③** + 评估模块（含**记分卡④**）+ OKX demo 实盘 + 文档/README/发布。
