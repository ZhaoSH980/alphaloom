...
> 10. **I1（T6 二审 Important）修复——Layer 1 私有属性图逃逸（`ctx._ctx.llm`）**：原 `_RestrictedContext.__slots__=("_ctx",)` 把真 ctx 存成沙箱可读的单下划线 slot；沙箱 AST 原只拦 dunder（`__x__`），`ctx._ctx.llm.chat()` 即经 `_ctx` 反向取回真 ctx 绕过 LLM 剥离（红队实测 offline 下 chat 被调）。原始 Critical C1 的 Layer 2 网关独立守住（LIVE+沙箱=409、hits=0），但 Layer 1"两层独立防御"名不副实，降级 Important 必修。**两条修复叠加**：① **沙箱 AST 根治（最通用）**——`visit_Attribute` 拒**一切下划线前缀属性访问**（`node.attr.startswith("_")` → SandboxError reason=`private_attr`），堵住"经私有 slot/内部属性反向取回受剥夺对象"整类逃逸；合法沙箱节点只对 inputs dict 做数值处理、访问自身非下划线属性（`self.factor`/`self.hist`），无正当理由碰任何下划线属性（**D3 沙箱 61 测试零误伤，实测锁定**——无一合法源码用 `._attr`）。③ **真 ctx 移出对象图（最彻底，防未来新增 slot 再犯）**——`_RestrictedContext` 弃 `_ctx` slot，真 ctx 存**模块级 `weakref.WeakKeyDictionary`**（键=受限视图弱引用、值=真 ctx 强引用），实例 `__dict__` 保持空、`view._ctx` 抛 AttributeError（对象图上真 ctx 不可达）；弱引用键使视图回收时映射自动清理无泄漏。红队测试 RED→GREEN：修复前 `ctx._ctx.llm.chat()` 注册成功 + 运行期 `spy.calls==1`；修复后注册期即 `SandboxError(private_attr)`，且即便 AST 被绕过对象图层 `view._ctx` 也取不回真 ctx（`spy.calls==0`）。变体覆盖 `ctx._ctx`/`ctx._anything.foo`/`ctx.__dict__`/`inputs._secret` 全拒。
> 11. **I2（T6 二审 Minor，顺手修）**：offline 下含"偷调 ctx.llm 的沙箱节点"的蓝图放行进回测、on_bar 触发 `SandboxEscapeError`，此前端点未捕获 → 通用 500。修为：`create_app` 注册 `@app.exception_handler(SandboxEscapeError)` → 干净 422 带解释（`{"error":"sandbox_escape",...}`），覆盖 leaderboard/ablation/evolve 全部同步 eval 端点。测试锁定：offline + thief 蓝图 → 422（<500）且 `spy.calls==0`（Layer 1 剥离生效证据）。
