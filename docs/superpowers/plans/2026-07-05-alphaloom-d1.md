...
13. **Task 7 审查移交（D3 前处理）**：①`check_stamped` 深递归化（list/嵌套 dict/Stamped.value 内部）——D3 LLM 节点引入结构化端口值前必须做，D1 节点集下不可达故缓；②candle 以共享引用发给同波节点（BarEvent 浅冻结），恶意节点可原地篡改污染下游——**Task 11 审查关注 CandleFeed 是否 emit copy**（Task 8 实测录制层有快照语义免疫，波内内存污染仍在）。
14. **Task 8 审查移交**：①**Engine 非崩溃安全**——step 异常后 _prev/节点状态/录制三重脏化（实测确认），使用契约=任何异常→run failed→实例弃用；D2 API 层落地该契约并加毒化标志（step 异常置 _dead，再 step 即抛）；②节点异常/on_pause 回调异常均裸传播中断 run——D2 API 捕获转 failed，WS 暂停回调必须自吞异常；③Recorder.fetch 无波内全序（索引扫描按 node_id 字典序，非执行序）——回放 UI 前改 `ORDER BY event_idx, rowid`；④不可序列化节点输出仅带 recorder 时炸（无 recorder 静默）——D3 与 13① 一并处理；⑤逐行 commit 性能税——长回测前批量事务或 WAL（Task 12 视性能）。
15. **Task 9 审查移交（PaperBroker）**：①**D2 前**：`submit()` 加 `qty <= 0 → return False`（防 qty=0 除零炸 run、负 qty 负费返现——恶意节点可直呼 broker）；②**D2**：`halt()` 与 pending 单语义定案（现状熔断后已挂单下根仍成交，最多多一根 bar 敞口；建议 halt 清空 _pending 并补测试）+ summary 进 API 边界时 `profit_factor: inf → None`（浏览器 JSON.parse 不认 Infinity；**CLI 的 JSON 输出也是对外边界，D2 一并收口**——Task 12 审查实测 win_rate=1.0 时 CLI 已输出裸 Infinity）；③**D4 前（接真实数据硬性）**：止损成交价 clamp——long `min(open, stop)` / short `max(open, stop)`（消除跳空乐观成交；D1 合成数据 open==prev close 无跳空故不可达）；④可选：entry fee 按 closed_qty 比例分摊、`new_qty` 判零改 `abs(...)<1e-12`（幽灵止损，D1 管线内 ExecuteOrder delta 归零精确故不可达）。
...
17. **Task 11 审查移交（对抗输入洞，D1 不可达，D2 前处理）**：①risk_gate 加 `isfinite(stop)/isfinite(qty)` 与 `qty<0` 检查（NaN stop 可穿透全链提交 NaN 卖单——与 15① qty guard 同族一起修；**终审复核追加：side 键整体缺失时 gates.py `sig["side"]` 自身 KeyError，一并防御**）；②sizer 加 `equity<=0 → hold`（负 equity 使 long 信号翻空，需无 kill_switch 图才可达）；③cross_signal 触零复燃（diff [+,0,+] 第三根再发 long）与 RSI 恒价=100 语义备查（D2 指标面板展示注意）。
...
19. **Stamped 解码契约（终审 I-3，归 D2 时间旅行任务）**：recorder 的 `{"__stamped__": as_of, "value": ...}` 编码全库无 decoder；D2 回放/时间旅行 UI 落地时实现 `from_json` 反解（注意 value 自含 `"__stamped__"` 键的歧义防护），并与 14③ 的 fetch 全序修复同批。另：CLI run 侧 CompileFailed 分支无测试（compile 侧已覆盖，两侧共享序列化；D2 API 边界重写时一并补）。
